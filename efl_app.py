...
if sys.platform == 'win32':
    try:
        ctypes.windll.dwmapi.DwmSetWindowAttribute(
            ctypes.windll.user32.GetParent(0),
            2,
...
def find_header_cols(ws, header_texts):
    """Find first column of each task's 3-column group."""
...
def find_user_rows(ws, iso_date, user_id):
    """Return {slot_name: row_index} for the given date + user."""
    rows = {}
...
def create_missing_rows(ws, iso_date, user_id, user_code):
    """Append 4 rows (3 daytime + OT) if date+user combo doesn't exist."""
...
def split_count(n):
    """Split n into 3 parts (50/30/20) with largest-remainder rounding."""
    if n <= 0:
        return [0, 0, 0]
...
def _apply_border_and_ot_highlight(ws, ot_row_indices, last_col):
    """Border every used cell + blue headers (rows 1-7) + yellow OT rows."""
    last_row = ws.max_row
...
def _find_last_task_col(ws):
    """Find last task's first column, return its LAST column (first + 2)."""
    last = 5
    for c in range(TASK_COL_START, ws.max_column + 1, TASK_COL_STEP):
        v = ws.cell(row=TASK_HEADER_ROW, column=c).value
...
def write_counts_to_excel(path, user_id, user_code, counts_by_day):
    """Write counts to Excel workbook."""
    try:
    ...
    details = []
    ot_rows_to_highlight = set()

    for iso_date, counts in counts_by_day.items():
        create_missing_rows(ws, iso_date, user_id, user_code)
        slot_rows = find_user_rows(ws, iso_date, user_id)
        if not slot_rows:
            details.append(f"⚠️  {iso_date} - no rows found for {user_id}")
            continue

        if OT_SLOT in slot_rows:
            ot_rows_to_highlight.add(slot_rows[OT_SLOT])

        any_written = False
        for task_label, cols_val in counts.items():
            header_texts = TASK_TO_HEADER.get(task_label)
            if not header_texts:
                continue
            cols = find_header_cols(ws, header_texts)
            if not cols:
                details.append(f"⚠️  {task_label} header not found in Excel")
                continue

            if isinstance(cols_val, (list, tuple)):
                val_list = list(cols_val)
            else:
                val_list = [cols_val]
            ...
            for val_idx, val in enumerate(val_list):
                if val_idx >= len(cols):
                    break
                col = cols[val_idx]
                parts = split_count(val)
                for slot_idx, slot_name in enumerate(DAYTIME_SLOTS):
                    if slot_name in slot_rows:
                        row_idx = slot_rows[slot_name]
                        ws.cell(row=row_idx, column=col).value = parts[slot_idx]
                any_written = True
    ...
    details.append("✔ Blue header (rows 1-7) applied")
...
def build_counts_for_range(values1, values2, user_id, start_date, end_date, count_sheet1_fn, sum_sheet2_fn):
    result = {}
    d = start_date
    while d <= end_date:
        ...
        day = {}
        for label in ('GDN Creation:', 'GRN Creation:', 'Load Plan or Asn:'):
            if label == 'GDN Creation:':
                day[label] = count_sheet1_fn(values1, 'GDN Creation:', user_id, date_str) + count_sheet1_fn(values1, 'GDN Reconciliation:', user_id, date_str)
            elif label == 'GRN Creation:':
                day[label] = count_sheet1_fn(values1, 'GRN Creation:', user_id, date_str) + count_sheet1_fn(values1, 'GRN Reconciliation:', user_id, date_str)
            else:
...
def compute_export_range(selected_date):
    """
    Rule:
      if selected.day == 1  -> previous month  (1st -> last day)
      else                  -> 1st of selected month -> (selected - 1 day)
    """
...
def generate_user_export(folder, user_id, user_code, selected_date, values1, values2,
    ...
    """Regenerate one user's workbook without mixing data from an older export."""
    ...
    try:
        ...
        if not ensure_excel_exists(temp_path):
            return False, "Could not create Excel template", path, []
        ok, message, details = write_counts_to_excel(temp_path, user_id, user_code, counts)
        if not ok:
            return False, message, path, details
        os.replace(temp_path, path)
        return True, "Excel export ready.", path, details
    except OSError as error:
        return False, f"Could not save Excel export: {error}", path, []
    finally:
    ...
    def connect(self):
        try:
            ...
            for cred_file in CREDENTIALS_FILES:
                for folder in (app_dir, resource_dir, os.getcwd()):
                    file_path = os.path.join(folder, cred_file)
                    if os.path.exists(file_path):
                        self.cred_file_used = file_path
                        break
                if self.cred_file_used:
                    break

            if not self.cred_file_used:
                return False, "Credentials file not found (looked for: credentials.json)"
    ...
    def search_and_delete_row_sheet1(self, search_column, search_value, user_id=None):
        ...
        try:
            ...
            for i, row in enumerate(all_values, 1):
                if len(row) > search_column and str(row[search_column]) == str(search_value):
                    if user_id is not None:
                        row_user = row[4] if len(row) > 4 else ''
                        if str(row_user).strip().lower() != str(user_id).strip().lower():
                            continue
                    self._rate_limit_wait()
    ...
    def search_and_delete_row_sheet2(self, search_column, search_value, user_id=None):
        ...
        try:
            ...
            for i, row in enumerate(all_values, 1):
                if len(row) > search_column and str(row[search_column]) == str(search_value):
                    if user_id is not None:
                        row_user = row[5] if len(row) > 5 else ''
                        if str(row_user).strip().lower() != str(user_id).strip().lower():
                            continue
                    self._rate_limit_wait()
...
class DatePicker:
    ...
    def prev_month(self, event=None):
        ...
        if month == 1:
            self.view_date = datetime(year - 1, 12, 1)
        else:
...
class NumericEntry(PlaceholderEntry):
    ...
    def is_filled(self):
        return not self._has_placeholder and self.get().strip() != ''


class PlaceholderCombobox(ttk.Combobox):
    def __init__(self, master=None, values=None, placeholder="", default_value=None, width=None, **kwargs):
        self.actual_values = [v for v in (values or []) if v != placeholder]
        self.placeholder = placeholder
        self.default_value = default_value if default_value is not None else (self.actual_values[0] if self.actual_values else None)
        self.placeholder_color = PLACEHOLDER_COLOR
        self.default_fg = TEXT_MAIN
        ...
        if self.default_value and self.default_value in self.actual_values:
            self._has_placeholder = False
        ...
        self.bind('<FocusOut>', self._on_focus_out)
        self.bind('<Map>', self._on_dropdown_open)
        self.bind('<Unmap>', self._on_dropdown_close)

    def _on_dropdown_open(self, event=None):
        self._is_dropdown_open = True

    def _on_dropdown_close(self, event=None):
        self._is_dropdown_open = False
    ...
    def _on_select(self, event=None):
        if self.get() == self.placeholder:
            self._has_placeholder = True
            self.config(foreground=self.placeholder_color)
        elif self.get() in self.actual_values:
            self._has_placeholder = False
            self.config(foreground=self.default_fg)

    def _on_focus_out(self, event=None):
        if not self._is_dropdown_open:
            if self.get() not in self.actual_values or self.get() == self.placeholder:
                if self.default_value and self.default_value in self.actual_values:
                    self.set(self.default_value)
                    self._has_placeholder = False
                    self.config(foreground=self.default_fg)
    ...
    def get_value(self):
        if self._has_placeholder or self.get() == self.placeholder:
            return ""
        return self.get()
    ...
    def is_filled(self):
        return not self._has_placeholder and self.get() in self.actual_values
...
class EFLApp:
    def __init__(self, root, container=None, standalone=True, profile=None, on_open_settings=None):
        ...
        profile = profile if profile is not None else load_saved_profile()
        self.user_id, self.user_code = profile if profile else (None, None)
        self._sheets_started = False
        self.selected_date = datetime.now()
        ...
        if self.user_id:
            self._start_sheets()
        self.root.after(200, self._animate_header)

    def _start_sheets(self):
        if not self._sheets_started:
            self._sheets_started = True
            self.root.after(50, self._init_google_sheets_async)

    def set_profile(self, profile):
        """Apply Settings changes to an already-open Tool 4."""
        ...
        for row in list(self.rows_top.values()) + list(self.rows_bottom.values()):
            row['count_label'].config(text='—')
        self._update_profile_ui()
        if self.user_id:
            self._start_sheets()
    ...
    def _update_profile_ui(self):
        configured = bool(self.user_id and self.user_code)
        if hasattr(self, 'user_label') and self.user_label.winfo_exists():
            self.user_label.config(text=self.user_id or "Not configured")
        for row in list(self.rows_top.values()) + list(self.rows_bottom.values()):
            row['submit_btn'].config(state='normal' if configured else 'disabled')
            row['delete_btn'].config(state='normal' if configured else 'disabled')
        if hasattr(self, 'download_btn') and self.download_btn.winfo_exists():
            self.download_btn.config(state='normal' if configured else 'disabled')
        if configured:
            if hasattr(self, '_setup_overlay') and self._setup_overlay.winfo_exists():
                self._setup_overlay.destroy()
            return

        self._setup_overlay = tk.Frame(self.root, bg=BASE_BG)
        self._setup_overlay.place(x=0, y=0, relwidth=1, relheight=1)

        card = tk.Frame(self._setup_overlay, bg=CARD_BG, highlightbackground=CARD_BORDER, highlightthickness=1, padx=32, pady=28)
        card.place(relx=0.5, rely=0.45, anchor='center')

        badge = tk.Frame(card, bg='#fef3c7', padx=8, pady=3)
        badge.pack(pady=(0, 12))
        tk.Label(badge, text="✦ PROFILE SETUP REQUIRED", bg='#fef3c7', fg=AURORA_AMBER, font=('Segoe UI', 8, 'bold')).pack()

        tk.Label(card, text="Set up your KPI profile", bg=CARD_BG, fg=TEXT_MAIN, font=('Segoe UI', 16, 'bold')).pack(pady=(0, 8))
        tk.Label(
            card,
            text="Enter your registered User ID and User Code in EFL NEXUS Settings to use KPI entry and download.",
            bg=CARD_BG, fg=TEXT_MUTED, font=('Segoe UI', 10), wraplength=480, justify='center'
        ).pack(pady=(0, 20))

        if self.on_open_settings:
            btn = tk.Button(
                card, text="Open Settings", command=self.on_open_settings,
                bg='#0d1b2a', fg='#ffffff', activebackground='#162e4c', activeforeground=AURORA_CYAN,
                font=FONT_BUTTON, relief='flat', padx=18, pady=8, cursor='hand2'
            )
            btn.pack()
        ...
        try:
            ...
            for attr in (20, 19):
                ...
                if result == 0:
                    ...
                    return
            print("Dark title bar not supported")
        except Exception as e:
    ...
    def _connect_thread(self):
        try:
            ...
            ok, msg = self.gs_manager.connect()
            if ok:
    ...
    def _on_connected(self):
        ...
        self._show_message("Connected to Google Sheets!", MSG_SUCCESS)
        self._start_auto_refresh()
        self._refresh_all_counts()

    def _on_disconnected(self):
        self._update_status(False)
        self._show_message("Failed to connect to Google Sheets", MSG_ERROR)
            ...
            if hasattr(self, 'status_indicator') and hasattr(self, 'status_text'):
                if connected:
                    ...
                    if hasattr(self, 'status_pill'):
                        self.status_pill.config(bg=STATUS_PILL_ONLINE_BG, highlightbackground=STATUS_PILL_ONLINE_BORDER)
                else:
                    self.status_indicator.config(fg=STATUS_PILL_OFFLINE_FG)
                    self.status_text.config(text='Offline', fg=STATUS_PILL_OFFLINE_FG)
                    if hasattr(self, 'status_pill'):
                        self.status_pill.config(bg=STATUS_PILL_OFFLINE_BG, highlightbackground=STATUS_PILL_OFFLINE_BORDER)
            if self.gs_manager:
    ...
    def _schedule_next_auto_refresh(self):
        if self._auto_refresh_id:
            try:
                ...
                pass
        self._auto_refresh_id = self.root.after(self._auto_refresh_interval * 1000, self._auto_refresh_tick)
    ...
    def close(self):
        """Stop recurring callbacks before the host window is destroyed."""
        self._anim_running = False
        for callback_id in (self._auto_refresh_id, self._msg_after_id):
            if callback_id:
                try:
    ...
    def _animate_header(self):
        """Retained for backwards-compatibility; header now uses standard Obsidian banner design."""
        pass
    ...
    def on_date_selected(self, date_obj):
        self.selected_date = date_obj
        today = datetime.now().date()
        selected = date_obj.date()
        self._is_past_date = (selected < today)
        if self._is_past_date:
            self._show_message("Past date selected (View Only)", MSG_WARNING)
        else:
            self._clear_message()
        self._cache_time = 0
        self._refresh_all_counts()
    ...
    def _make_row(self, parent, label_text, has_extra_counts=False, has_lp_count=False, dropdown_options=None):
        ...
        if has_extra_counts:
            ...
            combo = PlaceholderCombobox(right_frame, values=dropdown_options, placeholder='Select status', width=11)
    ...
    def _build_top_panel(self):
        ...
        top_configs = [
            ...
            ('GDN Creation:', ['New', 'Revise']),
            ('GRN Creation:', ['New', 'Revise']),
    ...
    def _is_duplicate_in_sheet1(self, task_name, job_id, job_status):
        ...
        try:
            ...
            status_norm = str(job_status).strip().lower() if job_status else ""
            ...
            for row in all_values[1:]:
                if not row or len(row) < 4:
                    continue
                row_task = _norm(row[1]) if len(row) > 1 and row[1] else ""
                row_job = str(row[2]).strip().lower() if len(row) > 2 and row[2] else ""
                row_status = str(row[3]).strip().lower() if len(row) > 3 and row[3] else ""
                row_user = str(row[4]).strip() if len(row) > 4 and row[4] else ""

                if row_task == task_norm and row_job == job_norm:
        ...
        try:
            ...
            for row in all_values[1:]:
                if not row or len(row) < 3:
                    continue
                row_job = str(row[2]).strip() if len(row) > 2 and row[2] else ""
    ...
    def _get_owner_of_row_sheet2(self, job_id):
        ...
        try:
            ...
            for row in all_values[1:]:
                ...
                row_job = str(row[2]).strip() if len(row) > 2 and row[2] else ""
                if row_job.lower() == job_norm:
                    row_user = str(row[5]).strip() if len(row) > 5 and row[5] else ""
                    return row_user if row_user else "another user"
            return None
        except Exception as e:
    ...
    def _refresh_all_counts(self):
        t = threading.Thread(target=self._refresh_counts_thread, daemon=True)
        t.start()
    ...
    def _refresh_counts_thread(self):
        ...
        try:
            ...
            if not user_id:
                return
            now = time.time()
            if self._sheet1_cache and (now - self._cache_time < self._cache_ttl):
                v1 = self._sheet1_cache
                v2 = self._sheet2_cache
            else:
                v1 = None
                v2 = None
                ...
                def fetch_sheet1():
                    nonlocal v1
                    if self.gs_manager and self.gs_manager.connected:
                        v1 = self.gs_manager.get_all_values_sheet1()

                def fetch_sheet2():
                    nonlocal v2
                    if self.gs_manager and self.gs_manager.connected:
                        v2 = self.gs_manager.get_all_values_sheet2()

                t1 = threading.Thread(target=fetch_sheet1)
                t2 = threading.Thread(target=fetch_sheet2)
                t1.start()
                t2.start()
                t1.join()
                t2.join()

                self._sheet1_cache = v1 or []
                self._sheet2_cache = v2 or []
                self._cache_time = now

            values1, values2 = self._sheet1_cache, self._sheet2_cache
            def apply_counts():
                if self.user_id == user_id:
                    self._update_counts_ui(values1, values2, selected_date, user_id)
                elif self.user_id:
                    self._refresh_all_counts()
    ...
    def _reset_all_rows(self):
        for row_data in self.rows_top.values():
            ...
            if row_data.get('combo'):
                row_data['combo'].reset()

        for row_data in self.rows_bottom.values():
            row_data['entry'].reset()
            for e in row_data.get('extra_entries', []):
                e.reset()

    def _validate_row_fields(self, row_data, is_bottom_panel):
        entry = row_data.get('entry')
        if not entry or not entry.is_filled():
            return False, "Job ID required"

        if is_bottom_panel:
            extra_entries = row_data.get('extra_entries', [])
            for e in extra_entries:
                if not e.is_filled():
                    return False, f"{e.placeholder} required"
        else:
            combo = row_data.get('combo')
            if combo and not combo.is_filled():
                return False, "Dropdown selection required"

        return True, "Valid"

    def _save_to_sheet1(self, section, data):
        if not self.user_id or not self.user_code:
            self._show_message("Set up your KPI profile in Settings", MSG_WARNING)
            return False
        if not self.gs_manager or not self.gs_manager.connected:
            self._show_message("Not connected to Google Sheets", MSG_ERROR)
            return False
        ts = self.selected_date.strftime('%d-%m-%Y %H:%M:%S')
        row = [ts, section, data.get('job_id', ''), data.get('job_status', ''), self.user_id]
        ok = self.gs_manager.append_row_sheet1(row)
        if ok:
            if is_reconciliation_task(section):
                try:
                    enqueue_job(data.get('job_id', ''), section, self.user_id)
                except Exception as e:
                    print(f"Could not queue reconciliation job: {e}")
            self._show_message("Saved to Sheet1", MSG_SUCCESS)
            self._cache_time = 0
            self._refresh_all_counts()
            return True
        else:
            self._show_message("Failed to save to Sheet1", MSG_ERROR)
            return False
    ...
    def _save_to_sheet2(self, section, data):
        ...
        row = [
            ...
            section,
            data.get('job_id', ''),
            data.get('load_id_count', ''),
            data.get('lp_count', ''),
            self.user_id
        ]
        ok = self.gs_manager.append_row_sheet2(row)
        if ok:
            self._show_message("Saved to Sheet2", MSG_SUCCESS)
            self._cache_time = 0
            self._refresh_all_counts()
            return True
        else:
            self._show_message("Failed to save to Sheet2", MSG_ERROR)
            return False
    ...
    def _delete_from_sheet1(self, job_id):
        ...
        if not self.gs_manager or not self.gs_manager.connected:
            self._show_message("Not connected to Google Sheets", MSG_ERROR)
            return False
        ok = self.gs_manager.search_and_delete_row_sheet1(2, job_id, self.user_id)
        if ok:
            self._show_message("Deleted from Sheet1", MSG_SUCCESS)
            self._cache_time = 0
            self._refresh_all_counts()
            return True
        else:
            owner = self._get_owner_of_row_sheet1(job_id)
            message = f"Cannot delete - belongs to {owner}" if owner and owner.lower() != self.user_id.lower() else "Job ID not found in Sheet1"
            self._show_message(message, MSG_ERROR)
            return False
    ...
    def _delete_from_sheet2(self, job_id):
        ...
        if not self.gs_manager or not self.gs_manager.connected:
            self._show_message("Not connected to Google Sheets", MSG_ERROR)
            return False
        ok = self.gs_manager.search_and_delete_row_sheet2(2, job_id, self.user_id)
        if ok:
            self._show_message("Deleted from Sheet2", MSG_SUCCESS)
            self._cache_time = 0
            self._refresh_all_counts()
            return True
        else:
            owner = self._get_owner_of_row_sheet2(job_id)
            message = f"Cannot delete - belongs to {owner}" if owner and owner.lower() != self.user_id.lower() else "Job ID not found in Sheet2"
            self._show_message(message, MSG_ERROR)
            return False

    def _on_submit(self, section, entry, extra_entries, combo):
        if not self.user_id or not self.user_code:
            self._show_message("Set up your KPI profile in Settings", MSG_WARNING)
            return
        if self._is_past_date:
        ...
        is_valid, msg = self._validate_row_fields(row_data, is_bottom)
        if not is_valid:
            self._show_message("Fill all fields to submit", MSG_ERROR, 2000)
            return
        ...
        with self._submit_lock:
            if is_bottom:
                load_val = extra_entries[0].get_value() if len(extra_entries) > 0 else ""
                lp_val = extra_entries[1].get_value() if len(extra_entries) > 1 else ""
                data = {
                    'job_id': job_id,
                    'load_id_count': load_val,
                    'lp_count': lp_val
                }
                existing_user = self._is_duplicate_in_sheet1(section, job_id, "")
                if existing_user:
    ...
    def _on_delete(self, section, entry, extra_entries, combo):
        if not self.user_id or not self.user_code:
            self._show_message("Set up your KPI profile in Settings", MSG_WARNING)
            return
        if self._is_past_date:
    ...
    def _on_refresh(self):
        if self._refresh_lock.locked():
            self._show_message("Refresh already in progress...", MSG_INFO, 1500)
            return
        if not self.gs_manager or not self.gs_manager.connected:
            self._show_message("Reconnecting...", MSG_INFO, 1500)
            t = threading.Thread(target=self._reconnect_thread, daemon=True)
            t.start()
        else:
            self._show_message("Refreshing counts...", MSG_INFO, 1500)
            self._cache_time = 0
            self._refresh_all_counts()
    ...
    def _on_download(self):
        if not self.user_id or not self.user_code:
            ...
            return
        if not self.gs_manager or not self.gs_manager.connected:
            self._show_message("Connect to Google Sheets before downloading", MSG_ERROR)
            return
        self._show_message("Preparing Excel export...", MSG_INFO, 2000)
        t = threading.Thread(target=self._do_download_thread,
                             args=(self.user_id, self.user_code, self.selected_date), daemon=True)
        t.start()

    def _do_download_thread(self, user_id, user_code, selected_date):
        try:
            start_date, end_date = compute_export_range(selected_date)
            range_str = f"{start_date.strftime('%d-%m-%Y')} to {end_date.strftime('%d-%m-%Y')}"

            v1 = self.gs_manager.get_all_values_sheet1() if self.gs_manager else []
            v2 = self.gs_manager.get_all_values_sheet2() if self.gs_manager else []
            if not v1 or not v2:
                self.root.after(0, lambda: self._show_message(
                    "Could not read KPI sheets; download cancelled", MSG_ERROR, 4000))
                return

            chosen_folder = [EXCEL_FOLDER.strip() if EXCEL_FOLDER and EXCEL_FOLDER.strip() else None]
            if not chosen_folder[0]:
                def pick_folder():
                    chosen_folder[0] = filedialog.askdirectory(title="Select folder for Excel file")
                self.root.after(0, pick_folder)
                while chosen_folder[0] is None:
                    time.sleep(0.05)

            if not chosen_folder[0]:
                self.root.after(0, lambda: self._show_message("Download cancelled", MSG_WARNING, 2000))
                return

            ok, message, path, details = generate_user_export(
                chosen_folder[0], user_id, user_code, selected_date, v1, v2,
                self._get_task_count_from_values_sheet1,
                self._get_sum_from_values_sheet2,
            )

            print("\n============================================================")
            print(f"[DOWNLOAD] Selected date: {selected_date.strftime('%d-%m-%Y')}")
            print(f"[DOWNLOAD] Export range : {range_str}")
            print(f"[DOWNLOAD] Folder       : {chosen_folder[0]}")
            ...
            if ok:
                self.root.after(0, lambda: self._show_message(f"Excel ready for {user_id}: {range_str}", MSG_SUCCESS, 4000))
            else:
                self.root.after(0, lambda: self._show_message(message, MSG_ERROR, 4000))
        except Exception as e:
            print(f"[DOWNLOAD] Error: {e}")
            import traceback
            traceback.print_exc()
            self.root.after(0, lambda: self._show_message(f"Download failed: {e}", MSG_ERROR, 4000))

    def _reconnect_thread(self):
        ok = False
        try:
            if self.gs_manager:
                ok, _ = self.gs_manager.connect()
        except Exception:
            ok = False
    ...
    def _on_reconnect_done(self, ok):
        ...
        if ok:
            self._show_message("Reconnected to Google Sheets!", MSG_SUCCESS)
            self._cache_time = 0
            self._refresh_all_counts()
        else:
            self._show_message("Failed to reconnect", MSG_ERROR)

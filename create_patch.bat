...
echo.
echo [2/3] Syncing latest assets, templates and AuditShip into dist\EFL_NEXUS...
...
if exist "Korber_AuditShip" (
    echo Syncing Korber_AuditShip to dist\EFL_NEXUS\Korber_AuditShip...
    robocopy "Korber_AuditShip" "dist\EFL_NEXUS\Korber_AuditShip" /E /COPY:DAT /DCOPY:DAT /R:2 /W:1 /NFL /NDL /NJH /NJS
    if errorlevel 8 (
        echo [ERROR] Failed to copy the Korber_AuditShip folder.
        pause
        exit /b 1
    )
)
